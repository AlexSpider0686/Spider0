          ...
          return (
            <div className="system-card" key={system.id}>
              ...
              <div className="zone-grid">
                ...
                <div className="vendor-hint full"><p>{selectedVendor.description}</p><button className="ghost-btn" type="button" onClick={() => refreshVendorPricing(system)}>Обновить цены</button></div>
                <VendorConfigurator
                  system={system}
                  onChange={(key, value) => updateSystem(system.id, "selectedEquipmentParams", { ...(system.selectedEquipmentParams || {}), [key]: value })}
                />

                {snapshot && <div className="pricing-caption full">Актуализация: {snapshot.fetchedAt ? new Date(snapshot.fetchedAt).toLocaleString("ru-RU") : "—"}</div>}
                ...
                <div className="vendor-hint full">Ед. цена: {rub(result?.equipmentData?.unitPrice || 0)} | Ключ выбора: {result?.equipmentData?.selectionKey || "fallback"}</div>
              </div>
            </div>
          );
        })}
