...
function dashboard(slide, plan) {
  ...
  plan.notes.forEach((item, i) => slide.addText(`• ${item}`, { x: 1.04, y: 3.62 + i * 0.48, w: 5.0, h: 0.26, fontFace: "Calibri", fontSize: 10.2, color: C.text, fit: "shrink" }));
  slide.addText("Принятые допущения", { x: 6.94, y: 3.24, w: 2.6, h: 0.16, fontFace: "Calibri", fontSize: 14, bold: true, color: C.ink });
  plan.timeline.assumptions.slice(0, 3).forEach((item, i) => slide.addText(`• ${item}`, { x: 6.96, y: 3.62 + i * 0.62, w: 5.2, h: 0.42, fontFace: "Calibri", fontSize: 10, color: C.text, fit: "shrink" }));
}
