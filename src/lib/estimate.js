...
function getSnapshotFactor(snapshot) {
  const entries = snapshot?.entries || [];
  if (!entries.length) return 1;
  const weighted = entries.reduce(
    (acc, entry) => {
      const fallback = toNumber(entry.fallbackPrice, 0);
      const price = toNumber(entry.price, fallback);
      const weight = Math.max(toNumber(entry.influenceWeight, 1), 0);
      if (!fallback || !weight) return acc;
      acc.weightedRatio += (price / fallback) * weight;
      acc.weights += weight;
      return acc;
    },
    { weightedRatio: 0, weights: 0 }
  );
  if (!weighted.weights) return 1;
  return Math.max(0.6, Math.min(1.8, weighted.weightedRatio / weighted.weights));
}

export function calculateSystem(system, zones, budget, marketSnapshot = null) {
  ...
  const marketSnapshotFactor = getSnapshotFactor(marketSnapshot);
  const vendorFactor = vendorCostRatio * technicalComplexityFactor * toNumber(system.customVendorIndex, 1) * marketSnapshotFactor;
  ...
  const regionCoef = toNumber(budget.regionCoef, 1);
  ...
  const equipmentData = calculateEquipment(system, zones, system.selectedEquipmentParams, fallbackUnitPrice, {
    hasSnapshot: Boolean(marketSnapshot?.entries?.length),
    snapshotFactor: marketSnapshotFactor,
  });
...
export function buildEstimateRows({ objectData, recalculatedArea, systemResults, totals }) {
  return [
    ...
    [],
    ["Система", "Вендор", "Параметры оборудования", "Цена за ед., ₽", "Кабель, м", "Ед. оборудования", "Материалы, ₽", "Труд, ₽", "Накладные+СИЗ+отчисления, ₽", "Прибыль, ₽", "НДС, ₽", "Итого, ₽"],
