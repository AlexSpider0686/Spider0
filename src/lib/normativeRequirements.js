...
function buildFireRequirements(context) {
  ...
  rows.push(
    createRequirement({
      systemType: "aps",
      title: "Для линий СПС и управляющих цепей, которые должны сохранять работоспособность при пожаре, требуется применение ОКЛ.",
      status: "mandatory",
      mandatory: true,
      source: NORMATIVE_SOURCES.sp484,
      reference: "разделы по кабельным линиям СПС и цепям управления противопожарной защиты",
      summary:
        "Платформа учитывает, что для АПС и связанных управляющих цепей на объекте требуется огнестойкое исполнение кабельных линий. Поэтому в нормативном контуре закладывается отдельный денежный вес на ОКЛ поверх базового кабельного состава.",
      rationale:
        "Для текущего проекта это особенно важно на линиях, обеспечивающих обнаружение пожара, передачу сигналов и запуск противопожарной автоматики. Если рабочая документация уточнит состав линий ОКЛ, коэффициент можно скорректировать, но нулевым он быть не должен.",
      appliedImpact: {
        oklCostPerMeter: 480,
        oklWorkFactor: 0.04,
      },
  ...
  rows.push(
    createRequirement({
      systemType: "soue",
      title: "Линии СОУЭ, сохраняющие работоспособность при пожаре, должны выполняться как ОКЛ.",
      status: "mandatory",
      mandatory: true,
      source: NORMATIVE_SOURCES.sp3,
      reference: "требования к линиям оповещения и управлению эвакуацией в составе противопожарной защиты",
      summary:
        "Для СОУЭ платформа добавляет нормативный вес ОКЛ, потому что линии оповещения и управляющие цепи должны сохранять функцию в условиях пожара. Это отражается отдельным влиянием на бюджет по длине трасс и сопутствующим монтажным работам.",
      rationale:
        "На текущем объекте этот резерв нужен для кабельных линий оповещения, линий управления и участков, от которых зависит выполнение сценария эвакуации. При детальной РД состав ОКЛ можно уточнить по конкретным трассам и зонам.",
      appliedImpact: {
        oklCostPerMeter: 520,
        oklWorkFactor: 0.05,
      },
