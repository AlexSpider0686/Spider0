    ...
    try {
      ...
      setAddressVerification({
        ...
        message: result.preview?.isMapFallback
          ? "Адрес подтверждён. Показана карта проверенной точки, потому что надёжное фото именно этого здания не найдено."
          : "Адрес подтверждён. Показано фото, которое прошло строгую проверку совпадения с адресом.",
        result,
      });
    } catch (error) {
      setAddressVerification({
